...
public class VoyageController {
    ...
    public ResponseEntity<?> startEmptyVoyage(@RequestBody StartEmptyVoyageRequest request) {
        try {
            VoyageResponse voyage = voyageServiceClient.startEmptyVoyage(request);
            return ResponseEntity.ok(voyage);
        } catch (Exception e) {
            String msg = e.getMessage();
            if (msg != null && msg.contains("\"message\"")) {
            ...
            return ResponseEntity
                    .badRequest()
                    .body(java.util.Map.of("message", msg != null ? msg : "Failed to start empty voyage"));
        }
    }
